...
- 固定发布页：`https://github.com/qiuC123/qiuC-tools/releases/tag/wechat-oa-v0.7.2`
- Windows x64 ZIP：`https://github.com/qiuC123/qiuC-tools/releases/download/wechat-oa-v0.7.2/wechat-oa-0.7.2-windows-x64.zip`
- SHA-256 文件：`https://github.com/qiuC123/qiuC-tools/releases/download/wechat-oa-v0.7.2/wechat-oa-0.7.2-windows-x64.zip.sha256`
- 固定 SHA-256：`fd739fd0846effb0f0de02063cc5d60c3e1335a1cc65b476b3d6bffbe5c6cfaa`
...
$releaseTemp = Join-Path ([System.IO.Path]::GetTempPath()) `
  ("wechat-oa-0.7.2-" + [guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $releaseTemp | Out-Null
$releaseZip = Join-Path $releaseTemp "wechat-oa-0.7.2-windows-x64.zip"
$releaseHash = "$releaseZip.sha256"
Invoke-WebRequest `
  "https://github.com/qiuC123/qiuC-tools/releases/download/wechat-oa-v0.7.2/wechat-oa-0.7.2-windows-x64.zip" `
  -OutFile $releaseZip
Invoke-WebRequest `
  "https://github.com/qiuC123/qiuC-tools/releases/download/wechat-oa-v0.7.2/wechat-oa-0.7.2-windows-x64.zip.sha256" `
  -OutFile $releaseHash
...
$pinnedHash = "fd739fd0846effb0f0de02063cc5d60c3e1335a1cc65b476b3d6bffbe5c6cfaa"
...
$releaseExe = Join-Path $releaseTemp `
  "wechat-oa-0.7.2-windows-x64\wechat-oa.exe"
& $releaseExe --version
& $releaseExe --json doctor
```

必须同时看到版本 `0.7.2`、进程退出码 `0` 和 Doctor JSON 的 `ok: true` 才算临时
...
只有 `wechat-oa media --help` 已列出 `doctor` 时，才运行 0.6.0 的纯本地媒体检查：
