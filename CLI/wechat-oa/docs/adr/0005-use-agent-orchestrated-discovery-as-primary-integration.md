...
Direct Discovery through wxcli's implemented Brave Discovery Provider remains an optional deterministic path for environments that do not invoke an agent. This decision supplements [ADR-0004](0004-separate-discovery-candidates-from-article-evidence.md): Candidate and Evidence separation applies equally to direct and agent-orchestrated discovery. [ADR-0009](0009-support-native-exa-direct-discovery.md) later adds Exa as a second native Direct Discovery Provider for callers that invoke wechat-oa directly; agent-orchestrated Candidate Ingestion remains supported.
