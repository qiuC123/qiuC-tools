# WeChat OA Windows x64 安装、升级与清理
...
1. 校验下载的 ZIP：
   ...
   ```powershell
   Get-FileHash .\wechat-oa-0.7.3-windows-x64.zip -Algorithm SHA256
   Get-Content .\wechat-oa-0.7.3-windows-x64.zip.sha256
   ```

   两处 SHA-256 应完全相同。

2. 解压整个 ZIP。`wechat-oa.exe`、兼容命令 `wxcli.exe` 和 `_internal` 目录必须保持在一起，不能只复制 EXE。维护者从仓库构建时，完整目录位于 `dist\release\wechat-oa-<版本>-windows-x64`。
3. 在仓库根目录运行安装脚本：

   ```powershell
   pwsh .\scripts\install-release.ps1 -Version 0.7.3
   ```
...
```powershell
$wechatOaExe = Join-Path $env:LOCALAPPDATA 'Programs\wxcli\current\wechat-oa.exe'
.\scripts\live-discovery-smoke.ps1 -Query '校园招聘' -WxcliPath $wechatOaExe -AllowLiveSearch
```
...
# 再授权 wechat-oa 回读微信原文；只有额外加 -AllowBrowser 才可能打开 Chrome
.\scripts\live-agent-discovery-smoke.ps1 -Query '校园招聘' -CodexPath 'codex' `
  -WxcliPath $wechatOaExe `
...
把新版本的完整产物放入 `dist\release\wechat-oa-<版本>-windows-x64`，然后运行：
...
只保留一个 `previous`：再次安装新版本时，原有 `previous` 会先删除，当前服役版本成为新的 `previous`。运行状态仍保存在兼容路径 `%LOCALAPPDATA%\wxcli`，不会因为产品改名、程序目录切换或回滚而丢失。回滚到 0.5.0 或更早版本时，旧程序继续使用同一状态目录。

## 回滚

先关闭正在运行的 wechat-oa，再执行：
...
先关闭所有 wechat-oa 和它打开的 Chrome 窗口，然后按需要执行：
...
随后可以删除 `%LOCALAPPDATA%\Programs\wxcli`，并从用户级 PATH 人工删除 `%LOCALAPPDATA%\Programs\wxcli\current`。如不再需要 Agent Skill，也可以删除 `%USERPROFILE%\.agents\skills\wechat-oa` 和兼容目录 `%USERPROFILE%\.agents\skills\wxcli`。程序没有安装系统服务，也不会创建外部 Release。

如果还要彻底删除账号配置：

1. 打开 Windows“凭据管理器”，删除 WeChat OA 使用的兼容 `wxcli` 凭据条目。
...
正式打包脚本需要 PowerShell 7 或更高版本，请用 `pwsh .\scripts\build-release.ps1` 运行。Windows 自带的旧 PowerShell 5.1 不能执行该构建脚本，但不影响已经打包好的 `wechat-oa.exe`。
