...
1. 校验下载的 ZIP：
   ...
   ```powershell
   Get-FileHash .\wechat-oa-0.7.1-windows-x64.zip -Algorithm SHA256
   Get-Content .\wechat-oa-0.7.1-windows-x64.zip.sha256
   ```
...
3. 在仓库根目录运行安装脚本：
   ...
   ```powershell
   pwsh .\scripts\install-release.ps1 -Version 0.7.1
   ```
