...
The implemented Direct Discovery Providers are Brave Web Search and Exa Search. Brave is the backward-compatible default and receives a mandatory `site:mp.weixin.qq.com/s` restriction. Exa makes one bounded `deep` search with `includeDomains: ["mp.weixin.qq.com"]`; using a host-only upstream filter avoids relying on provider-specific path matching. A structured natural-language main query leads with de-duplicated company and account hints, identifies them as Official Account preferences, includes the caller query and small lexical variants for common campus-recruiting shorthand, and expresses the requested publication window as soft guidance. Bounded `additionalQueries` cover caller wording, shorthand variants, recruiting-program wording, and publication year; a fixed search-planning prompt prefers direct source articles over other organizations, aggregators, reposts, interview stories, and generic roundups. It does not use mandatory exact-phrase quoting or send `startPublishedDate`/`endPublishedDate`, because hard provider filters can hide an indexed WeChat link when Exa lacks publication metadata. The requested window remains a candidate-ranking hint and is enforced only against a date observed from successful WeChat Hydration. In both providers, wxcli independently accepts only URLs that satisfy its strict HTTPS `mp.weixin.qq.com/s` contract. Agent-Orchestrated Discovery remains supported and may call an External Discovery Provider before submitting a Candidate Batch. Every path records real search provenance and none is presented as official WeChat search.
...
Each complete bounded provider page is ranked from sanitized search metadata before the public candidate limit is applied. Exact and related expected-account hints are preferred; an explicit different-account hint is demoted but never rejected. Company/query coverage and provider date hints refine ordering, while Search Provenance preserves the provider's original rank. Candidates are then strictly URL-validated, deduplicated, and ranked again before Hydration. When Hydration is enabled, up to the first 10 ranked candidates are priority attempts. Additional candidates may be selected for expected-account, company, keyword, recency, uncertainty, or source-diversity reasons, but no more than 20 candidates are attempted per invocation. These are generic relevance decisions, never recruitment-batch classifications or identity/date evidence.

The private Exa retrieval-strategy version participates in the query fingerprint. An upgrade that changes search planning therefore does not reuse an older Exa candidate-page cache or accept older Exa continuation tokens; Brave fingerprints and the public schema remain unchanged. This is cache invalidation, not a cache clear, and it does not remove candidate history or credentials.
...
- After separate user authorization, `browser verify ARTICLE_URL` may open one challenged strict Public URL in the dedicated visible profile for up to five minutes and return Article Evidence after manual verification. Calling agents never take over that window through computer-use or generic browser control.
