...
- wechat-oa 提取公众号显示名、公开稳定 `biz_id`、正文外链、图片 URL 清单和稳定哈希，但不访问正文中的官网或 ATS，也不判断企业、招聘批次或岗位。0.6.0 的 Media Evidence 数据模型基础已经实现；图片下载、二维码、OCR 和 Evidence Bundle 仍未接入运行时，0.5.1 命令不提供这些能力。
