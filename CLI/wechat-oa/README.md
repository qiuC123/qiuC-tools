...
[`wechat-oa-v0.7.3`](https://github.com/qiuC123/qiuC-tools/releases/tag/wechat-oa-v0.7.3).
...
0.7.3 将 Exa 召回升级为单次 deep 多查询：结构化主查询、软发布日期窗口和补充查询共同
扩大语义召回；搜索策略版本进入缓存指纹，升级后不复用旧策略缓存。外部 CLI 契约不变。
...
.\dist\release\wechat-oa-0.7.3-windows-x64\wechat-oa.exe --version
Get-FileHash .\dist\release\wechat-oa-0.7.3-windows-x64.zip -Algorithm SHA256
```
