...
- 当前分支实现 QR/OCR 分析编排：对下载结果做防御性复验，相同图片字节按 SHA-256 只分析一次并映射回每个文章图片位置；QR 与 OCR 失败互相隔离，错误哈希/语言关联被替换为稳定失败证据；每批 OCR 输出总字符数受配置硬上限约束；下载省略数进入 Media Evidence、摘要和稳定哈希。派生结果缓存、CLI 控制和 Evidence Bundle 仍未实现。
