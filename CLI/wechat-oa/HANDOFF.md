...
- 当前开发分支：`codex/media-analysis-orchestration`，基于已经合并 PR #7 的 `main`。
...
- 当前分支实现 QR/OCR 分析编排：对下载结果做防御性复验，相同图片字节按 SHA-256 只分析一次并映射回每个文章图片位置；QR 与 OCR 失败互相隔离，错误哈希/语言关联被替换为稳定失败证据；调用方降低后的图片、QR、OCR 和文章限额由编排层再次强制执行；下载省略数进入 Media Evidence、摘要和稳定哈希。派生结果缓存、CLI 控制和 Evidence Bundle 仍未实现。本地完整验证为 356 项 pytest 全绿，mypy 检查 44 个源文件无错误；临时 PyInstaller onedir 构建和版本/JSON/帮助离线冒烟通过，Pillow 与 `zxingcpp.pyd` 已进入产物。正式构建脚本因本地已有 0.5.1 release 而按设计拒绝覆盖，既有正式发布包未改动。

### 下一步计划

按以下顺序推进，避免同时做版本治理和新功能：

1. 0.6.0 的 Media Evidence 数据模型、安全图片下载器、原始字节 Media Cache、文章级数量/总字节编排、本地标准二维码分析器、可替换 Windows OCR Provider 和 QR/OCR 分析编排已经完成；下一步设计并接入默认关闭、显式启用的 CLI 控制，再实现可审计的 Evidence Bundle。
